export default class Processor {
  ...
  // TODO What about sid?
  process(schema, data, _id) {
    let id;
    if (_id == null) id = data.id;
    let schemaResolved = this.resolveSchema(schema);
    let result = schemaResolved.call(this, data);
    if (id != null) this.namespace[id] = result;
    return result;
  }
  resolve(schema, id) {
    if (id.charAt(0) === '#') return this.resolve(schema, id.slice(1));
    if (this.namespace[id] != null) return this.namespace[id];
    return this.process(schema, this.namespacePre[id], id);
  }
}
